...
export const Project4 = () => {
  ...
  // It will pass data to the ToDoLocalStorage.jsx file
  ...
  function handleDeleteToDo(clickedItem) {
    ...
    setTask((prevTask) =>
      prevTask.filter((task) => task.content !== clickedItem)
    );
  }
      ...
      <section className="todo-container">
        ...
        <ul>
          {task.map((currTask) => {
            return (
              <ToDoList
                onDeleteBtn={handleDeleteToDo}
                index={currTask.id}
                currTask={currTask.content}
                onCheckedBtn={handleCheckedToDo}
